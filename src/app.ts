...
export class App {
  ...
  constructor() {
    ...
    const wlogger = logging("winston")
    this.app.use(
      morgan("dev", {
        logger: wlogger.debug.bind(wlogger),
        colored: true,
      })
    )
  ...
  public startSecureSever() {
    ...
    return https.createServer(options, this.app.callback()).listen(PORT, () => {
      logger.debug(`[${NODE_ENV}] Koa HTTPS server started at port: ${PORT}`)
    })
  }

  public startHTTPServer() {
    return this.app.listen(PORT, () => {
      logger.debug(`[${NODE_ENV}] Koa HTTP server started at port: ${PORT}`)
    })
  }
